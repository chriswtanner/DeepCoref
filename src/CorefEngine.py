...
class CorefEngine:
	if __name__ == "__main__":
		...
		stoppingPoints = [0.15,0.17,0.19,0.21,0.23,0.26,0.28,0.301,0.32,0.34,0.37,0.39,0.401,0.41,0.42,0.43,0.44,0.45,0.46,0.47,0.48,0.49,0.501,0.51,0.52,0.53,0.55,0.57,0.601,0.62,0.64,0.66,0.68,0.701,0.72,0.74,0.76,0.78,0.801,0.81]
		...
		if runFFNN: # deep clustering approach
			...
			for sp in stoppingPoints:
				...
				if args.useECBTest: # use corpus' gold test set
					...
					(bcub_p, bcub_r, bcub_f1, muc_p, muc_r, muc_f1, ceafe_p, ceafe_r, ceafe_f1, conll_f1) = get_conll_scores(goldenClusters, predictedClusters)
					print("AGG F1 sp:",str(sp),"=",str(conll_f1))
				else:
