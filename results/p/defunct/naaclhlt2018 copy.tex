...
\subsection{Loss}
The goal of our model is to maximize discriminability between mentions belongings to different events, while enforcing features to be as similar as possible when they are of the same class.  Contrastive Loss is perfectly suited for this objective \cite{SchroffKP15,pmlr-v48-liud16}, as shown in Equation \ref{eq:contrastive}.  Because our input is mention pairs, there is a strong class imbalance, so we down-sample the negative examples, yielding a training set of 5 negative examples per positive example.
